...
import {
  ...
  APIURL_GET_PROVINES,
  APIURL_GET_REGIONES,
  APIURL_POST_ADDRESS_BY_TECHNICIAN,
...
const AddAddress: FunctionComponent<AddAddressProps> = ({ userName, handleClickNext, id }) => {
  ...
  const GetTitles = () => {
    setLoading(true);
    httpRequest
      .getRequest<IOutputResult<ITitleResultModel[]>>(`${APIURL_GET_ADDRESS_TITLE}`)
      .then((result) => {
  ...
  const GetCountryList = () => {
    setLoading(true);
    httpRequest
      .getRequest<IOutputResult<ICountryResultModel[]>>(`${APIURL_GET_COUNTRIES}`)
      .then((result) => {
  ...
  const GetCityList = (provinesId: number) => {
    setLoading(true);
    httpRequest
      .getRequest<IOutputResult<ICitiesResultModel[]>>(`${APIURL_GET_CITIES}?ParentId=${provinesId}`)
      .then((result) => {
  ...
  const onSubmit = (data: IAddAddressModel) => {
    if (!userName) return toast.showError('مشتری انتخاب نشده');
    setLoading(true);
    const body = {
      userName: userName,
      countryId: countryId,
    ...
    if (data) {
      ...
      httpRequest
        .postRequest<IOutputResult<IAddAddressesResultModel>>(APIURL_POST_ADDRESS_BY_TECHNICIAN, body)
        .then((result) => {
          result.data.isSuccess ? toast.showSuccess(result.data.message) : toast.showError(result.data.message);
          handleClickNext(id, userName, 2, result.data.data.refkey);
          setLoading(false);
        })
        .finally(() => {
          setLoading(false);
        ...
        <Form onSubmit={handleSubmit(onSubmit)}>
          <div className="h-100-55" style={{ marginBottom: '0px' }}>
            ...
            <Button type="submit" className="primary-btn">
              {loading ? <LoadingComponent /> : 'ثبت آدرس و ادامه'}
            </Button>
          </div>
        </Form>
      </div>
